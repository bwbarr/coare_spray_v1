...
def ssgf_dissejec(SSGFname,eps,swh,dcp_in,mss_in,ustar,z0,L,gf,r0=None,delta_r0=None,extravars=None):
    """
    Dissipation-ejection-based SSGF, based on Fairall et al. (2009) and updated in Barr et al. (2023).  Returns 
    ...
    Parameters:
        SSGFname - string naming which SSGF to use.  Options are:
            Seastate-based: 'dissejec_SS_BCF23','dissejec_SS_C3.6_CtlSpr','dissejec_SS_C3.6_OptSpr','dissejec_SS_BEA26_OptSpr'
            Wind-based:     'dissejec_Wi_BCF23','dissejec_Wi_C3.6_CtlSpr','dissejec_Wi_C3.6_OptSpr','dissejec_Wi_BEA26_OptSpr',
                            'dissejec_Wi_C2optimize'
        eps - wave energy dissipation flux [W m-2], not used for wind-only models
        swh - significant wave height [m], not used for wind-only models
        dcp_in - dominant phase velocity [m s-1], suffix '_in' prevents input from being overwritten, not used for wind-only models
        mss_in - mean squared waveslope [-], suffix '_in' prevents input from being overwritten, not used for wind-only models
        ustar - friction velocity including gustiness [m s-1]
        z0 - momentum roughness length [m]
        L - Obukhov stability length [m]
        gf - gust factor [-]
        r0 - SSGF radius vector [m]
        delta_r0 - SSGF bin width vector [m]
        extravars - dictionary of variables for special options (used by 'dissejec_Wi_C2optimize')
    Return:
        r0 - SSGF radius vector [m]
        delta_r0 - SSGF bin width vector [m]
    ...
    elif SSGFname in ['dissejec_SS_C3.6_CtlSpr','dissejec_Wi_C3.6_CtlSpr']:
        model_coeffs = [2.2,0.969,0.1116            ,0.719,2.17,0.852]
    elif SSGFname in ['dissejec_SS_C3.6_OptSpr','dissejec_Wi_C3.6_OptSpr']:
        model_coeffs = [2.2,0.969,0.0759            ,0.719,2.17,0.852]    # To be finalized
    elif SSGFname in ['dissejec_SS_BEA26_OptSpr','dissejec_Wi_BEA26_OptSpr']:
        model_coeffs = [2.2,0.969,0.120             ,0.719,2.17,0.852]    # To be finalized
    elif SSGFname in ['dissejec_Wi_C2optimize']:
        model_coeffs = [2.2,0.969,extravars['C2_DE'],0.719,2.17,0.852]    # C2 is user-specified for optimization
    ...
    if   SSGFname in ['dissejec_SS_BCF23','dissejec_SS_C3.6_CtlSpr','dissejec_SS_C3.6_OptSpr','dissejec_SS_BEA26_OptSpr']:    # Seastate-dependent
    ...
    elif SSGFname in ['dissejec_Wi_BCF23','dissejec_Wi_C3.6_CtlSpr','dissejec_Wi_C3.6_OptSpr','dissejec_Wi_BEA26_OptSpr',\
                      'dissejec_Wi_C2optimize']:    # Wind-dependent
...
def ssgf_whitecap(SSGFname,wspd10,r0=None,delta_r0=None):
    """
    ...
    Parameters:
        SSGFname - string naming which SSGF to use.  Options are: 
            Seastate-based: None
            Wind-based: 'whitecap_Wi_MOM80', 'whitecap_Wi_BCF23_published', 'whitecap_Wi_BCF23_fixed'
    ...
    if SSGFname in ['whitecap_Wi_MOM80']:
        fs = 0.4
    ...
    elif SSGFname in ['whitecap_Wi_BCF23_published','whitecap_Wi_BCF23_fixed']:
        if SSGFname == 'whitecap_Wi_BCF23_published':
            fs = 3.1
        elif SSGFname == 'whitecap_Wi_BCF23_fixed':
            fs = 2.2
    ...
    # The line below used to contain a major bug where r0 was used instead of r0_micrometers.  The bug added
    ...
    # current range of uncertainty for spray generation.  Versions of the whitecap and dissejec spray
    # generation models are available to test the published and corrected variants resulting from the bug.
